...
object Process2 {
  ...
  def Setup(): (Map[Int, StaticCellInfo1], Map[String, Int]) ={

    val tmpCellInfo = Map[Int, StaticCellInfo1]()
  ...
  def reduceProcess(key : String, Iter : Iterable[String], cellmap : Map[Int, StaticCellInfo1], neimap : Map[String, Int]): String = {
    ...
    if(key.equals("-1")){
      ...
      val a = vlist.map{e =>
        ...
        xdr.nei_freq_ = if (e(16) != "") e(16).toLong else -1L
        xdr.nei_rsrp_ = if (e(18) != "") e(18).toLong else -1L
  ...
  def locate(sdata : XDR_UE_MR_S1, udata : User1) : (Double, Double, User1, Int) ={
    var aoa = sdata.aoa_
    aoa match{
      //如果数据没有aoa信息
      case -1 => {
  ...
  def locate_by_ta_aoa(aoa : Int, ta : Int, cell_id : Int) : (Double, Double, Int) ={
    ...
    val cellinfo_ = CellInfo.getOrElse(cell_id, new StaticCellInfo1)
  ...
  def locate_by_nei_level(sdata : XDR_UE_MR_S1) : (Double, Double, Int) ={
    var longitude_ : Double = -1
    var latitude_ : Double = -1
    val cellId = sdata.cell_id_
    val main_cell = CellInfo.getOrElse(cellId, new StaticCellInfo1)
            ...
            main_cell.in_door_ match{
              ...
              case _ => {
                ...
                neiinfo_cellid match{
                  ...
                  case _ => {
                    val neiinfo = CellInfo.getOrElse(neiinfo_cellid, new StaticCellInfo1)
  ...
  def GetInsideMRLonLat(main_cell : StaticCellInfo1): (Double, Double) = {
    // 室内计算
    var longitude_ : Double = -1
    var latitude_ : Double = -1

...
//    Process.init
  ...
  def locate(sdata : XDR_UE_MR_S, udata : User) : (Double, Double, User) ={
    var aoa = sdata.aoa_
    aoa match{
      //如果数据没有aoa信息
      case -1 => {
    ...
    res._1 match{
      case -1 => {
        ...
        (res1._1, res1._2, udata)
      }
      case _ => (res._1, res._2, udata)
    }
  }

  def locate_by_ta_aoa(aoa : Int, ta : Int, cell_id : Int) : (Double, Double) ={
...
//        "  disper_latitude_=" + disper_latitude_ + " cellinfo_.longitude_=" +cellinfo_.longitude_ + " cellinfo_.latitude_="+cellinfo_.latitude_)
  ...
  def locate_by_nei_level(sdata : XDR_UE_MR_S) : (Double, Double) ={
...
//                println("neiinfo_key="+neiinfo_key + " nei_cell_pci=" + sdata.nei_cell_pci_ + "  nei_freq_=" + sdata.nei_freq_)
                neiinfo_cellid match{
                  case -1 => (longitude_, latitude_)    //临区信息在redis表中未找到
                  case _ => {
                    val neiinfo = CellInfo.getOrElse(neiinfo_cellid, new StaticCellInfo)
                    neiinfo.cellid_ match{
                      case -1 => (longitude_, latitude_)
                      case _ => if (sdata.nei_rsrp_ == -1 || sdata.serving_rsrp_ > 97) (main_cell.longitude_, main_cell.latitude_)
                                  else GetLocInfo(sdata, main_cell, neiinfo)
                    }
                  }
                }
              }

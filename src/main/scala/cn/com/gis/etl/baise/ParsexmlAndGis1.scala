...
object ParsexmlAndGis1 {
  def parseXML1(content: String): Seq[String] ={
  ...
  def main(args: Array[String]):Unit={
    ...
    val result = sc.newAPIHadoopFile[LongWritable, Text, XmlInputFormat](args(0)).mapPartitions({x => x.flatMap(y => parseXML1(y._2.toString))})
    //    result.saveAsTextFile(args(2))
    val result1 = result.map(Process1.mapProcess).groupByKey().map(e => {Process1.reduceProcess(e._1, e._2, x.value, y.value)})

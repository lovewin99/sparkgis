...
import cn.com.gis.etl.shanghai.function.fingergis4
...
object zhiyaogis1 {
  ...
  def tmpInmapProcess(in: String): ((String, String, String), Array[String]) = {
    ...
    if(Finger_length == strArr.length){
      ...
      if(!outOfChina(lon, lat)){
        ...
        val sg = lon + "|" + lat
        val bsic = if(strArr(bsic_index).toInt < 10) "0"+strArr(bsic_index) else strArr(bsic_index)
        val flag = strArr(bcch_index) + bsic
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result = textRDD.mapPartitions{Iter => Iter.map{tmpInmapProcess}}.groupByKey().map{x => tmpreduceProcess(x._1, x._2)}.groupByKey().
      ...
      //      }.toArray
      Iter.map{x =>
        fingergis4.location(x._1, x._2, info.value)
      }
    }

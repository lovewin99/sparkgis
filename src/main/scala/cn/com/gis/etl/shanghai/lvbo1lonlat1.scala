          ...
          tdata.length match{
            ...
            case _ => {
              ...
              if(20 == tdata.length){
                ...
                //                val nlonlat = Mercator2lonlat(rint(sgx).toInt * grip_size, rint(sgy).toInt * grip_size)
                val d = sqrt(pow(tdata(10)(index_sgx).toDouble - sgx, 2) + pow(tdata(10)(index_sgy).toDouble  - sgy, 2))
                // 如果第10条数据大于门限值 则用中心坐标更新第10条坐标
                if(d > 100){
                  tdata(10).update(index_sgx, sgx.toString)
                  tdata(10).update(index_sgy, sgy.toString)

                  val nd = sqrt(pow(tdata(10)(index_lon).toDouble - sgx, 2) + pow(tdata(10)(index_lat).toDouble  - sgy, 2))

//                  val (nnlon, nnlat) = Mercator2lonlat(rint(sgx).toInt*20, rint(sgy).toInt*20)
//                  println(s"nnlon=$nnlon   nnlat=$nnlat")
//                  val nd = calc_distance(tdata(10)(index_srclon).toDouble, tdata(10)(index_srclat).toDouble, nnlon, nnlat)
                  tdata(10).update(index_dis, nd.toString)
                }
